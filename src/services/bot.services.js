...
const sincronizarStock = async () => {
  ...
  try {
    ...
    await Promise.all(
      Object.entries(stockMap).map(
        async ([nombre, { stock, mcItem, tipo }]) => {
          try {
...
bot.once("spawn", async () => {
  console.log(
    `[*] Bot "${config.bot.username}" conectado al servidor ${config.bot.host}`,
  );
...
const entregarObjeto = (username, items) => {
  ...
  const mensajeCompra = () => {
    ...
    for (const { itemName, cantidad } of items) {
      bot.chat(
        `/msg ${username} [!] Acabas de comprar un "${itemName}", iré hacia ti, no te muevas!`,
      );
    }
  };
  ...
  return new Promise((resolve, reject) => {
    ...
    if (username === bot.username)
      return reject("No me puedo vender a mi mismo");
    ...
    for (const { itemName, cantidad } of items) {
      ...
      bot.inventory.items().filter((item) => {
        ...
        } else if (item.displayName === itemName) {
          // Si el display name es igual al nombre del objeto
          itemsInventario.push(item);
          stock += item.count;
        }
      });
    ...
    const distanciaAlJugador = objetivo
      ? bot.entity.position.distanceTo(objetivo.position)
      : 999;
    ...
    const jugadorAlcanzado = async () => {
      try {
        ...
        for (const { itemName, cantidad } of items) {
          ...
          await sincronizarStock();
          console.log(
            `[!] Objeto "${itemName}" entregado a "${username}", stock sincronizado`,
          );
        }
    ...
    if (distanciaAlJugador > 150 || !objetivo) {
      ...
      setTimeout(async () => {
        ...
        const xFrente =
          objetivoActualizado.position.x -
          Math.sin(objetivoActualizado.yaw) * 3;
        const zFrente =
          objetivoActualizado.position.z -
          Math.cos(objetivoActualizado.yaw) * 3;
      ...
      bot.on("path_update", (results) => {
        ...
        if (results.status === "noPath" || results.status === "timeout") {
          ...
          bot.chat(
            `/msg ${username} [!] No pude llegar a tu ubicación, intenta un espacio abierto`,
          );
